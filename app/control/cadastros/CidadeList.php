...
class CidadeList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $uf_id->setMask('{sigla}');
        $descricao->setMask('{descricao}');

        $id->setSize('100%');
        $uf_id->setSize('100%');
        $descricao->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Código", null, '14px', null, '100%'),$id],[new TLabel("Descrição", null, '14px', null, '100%'),$descricao],[new TLabel("UF", null, '14px', null, '100%'),$uf_id]);
        ...
        $column_uf_sigla = new TDataGridColumn('uf->sigla', "UF", 'center' , '10%');
        ...
        $this->datagrid->addColumn($column_uf_sigla);
